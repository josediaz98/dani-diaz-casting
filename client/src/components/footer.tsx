...
function LiveClock() {
  ...
  return (
    <div className="flex flex-col">
      <span className="text-gray-500 uppercase tracking-widest text-xs mb-1">{t("footer.location")}</span>
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end pt-8 md:pt-12">
        <div className="space-y-4 max-w-4xl">
          <h2 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold leading-[0.9] tracking-tight">
            {t("footer.letsCreate")} <br />
            <span className="text-purple-400">{t("footer.somethingIconic")}</span>
          </h2>
        </div>
      ...
      <div className="relative z-10 grid grid-cols-2 md:grid-cols-12 gap-8 md:gap-12 border-t border-white/10 pt-8 md:pt-12 mt-auto">
        ...
        <div className="col-span-1 md:col-span-4 flex flex-col md:flex-row gap-8 text-xs md:text-sm uppercase tracking-widest text-gray-400">
          <div className="flex flex-col gap-3">
            <span className="text-white font-bold mb-1">{t("footer.sitemap")}</span>
            <Link href="/"><a className="hover:text-purple-400 transition-colors">{t("common.home")}</a></Link>
            <Link href="/work/fiction"><a className="hover:text-purple-400 transition-colors">{t("nav.work")}</a></Link>
            <Link href="/about"><a className="hover:text-purple-400 transition-colors">{t("nav.about")}</a></Link>
          </div>
          <div className="flex flex-col gap-3">
             <span className="text-white font-bold mb-1">{t("footer.socials")}</span>
        ...
        <div className="col-span-1 md:col-span-4 flex flex-col justify-between items-end h-full">
           ...
           <p className="text-xs text-gray-600 text-right">
             © {new Date().getFullYear()} Dani Díaz.<br/>{t("footer.rights")}
           </p>
        </div>
      </div>
    </footer>
